...
class TechnicalSignal:
    ...
    trend_strength: float = 0.0       # 0-1 how aligned MAs are
...
class FundamentalSignal:
    ...
    value_score: float = 5.0          # 0-10, Graham-style value assessment
    quality_score: float = 5.0        # 0-10, business quality (ROE, margins, cash flow)
    growth_score: float = 5.0         # 0-10, growth trajectory
    safety_score: float = 5.0         # 0-10, balance sheet safety (Graham margin of safety)
    peg_ratio: float | None = None
    intrinsic_value_estimate: float = 0.0  # rough DCF / earnings-based estimate
    margin_of_safety_pct: float = 0.0      # (intrinsic - current) / current


@dataclass
class QuantSignal:
    """Pre-computed quantitative trading parameters."""
    kelly_fraction: float = 0.0       # Kelly criterion: f* = (bp - q) / b
    half_kelly: float = 0.0           # half-Kelly for safety
    position_size_pct: float = 0.0    # recommended position %
    position_size_shares: int = 0     # recommended shares (100-lot)
    expected_return: float = 0.0      # annualized
    risk_reward_ratio: float = 0.0
    win_probability: float = 0.5
    avg_win_loss_ratio: float = 1.0
    composite_score: float = 5.0      # 0-10 weighted score
...
class RiskSignal:
    ...
    portfolio_var_impact: float = 0.0 # how this trade changes portfolio VaR
...
def compute_technical_signal(
    ...
    mas = [indicators.get(f"ma_{p}", 0) for p in [5, 10, 20, 60]]
    mas = [m for m in mas if m > 0]
    if len(mas) >= 3:
    ...
    elif bb_lower > 0 and current_price < bb_lower:
    ...
    if supports and current_price > 0:
        sig.support_distance_pct = round((current_price - supports[0]) / current_price * 100, 2)
    if resistances and current_price > 0:
        sig.resistance_distance_pct = round((resistances[0] - current_price) / current_price * 100, 2)
...
def compute_fundamental_signal(
    ...
    pe = _latest(financials.get("pe_ratio")) or 0
    pb = _latest(financials.get("pb_ratio")) or 0
    value_points = 5.0
    if pe > 0:
        if pe < 10:
            value_points += 2.5
    ...
    roe = _latest(financials.get("roe")) or 0
    gross_margin = _latest(financials.get("gross_margin")) or 0
    net_margin = _latest(financials.get("net_margin")) or 0
    ocf = _latest(financials.get("operating_cash_flow")) or 0
    net_profit = _latest(financials.get("net_profit")) or 0
    ...
    if net_profit > 0 and ocf > net_profit:
        quality += 1.0
    elif net_profit > 0 and ocf < net_profit * 0.5:
        quality -= 1.5

    sig.quality_score = max(0.0, min(10.0, quality))

    # ── Growth score ──
    rev_growth = _latest(financials.get("revenue_growth")) or 0
    profit_growth = _latest(financials.get("profit_growth")) or 0
    growth = 5.0
    ...
    debt_to_equity = _latest(financials.get("debt_to_equity")) or 0
    safety = 5.0
    ...
    # ── PEG ratio ──
    eps_growth = max(rev_growth, profit_growth)
    ...
    eps = _latest(financials.get("eps")) or 0
    if eps > 0:
        g = min(max(eps_growth, 0), 25)  # cap growth at 25%
        graham_value = eps * (8.5 + 2 * g)
        sig.intrinsic_value_estimate = round(graham_value, 2)
        if current_price > 0:
            sig.margin_of_safety_pct = round(
                (graham_value - current_price) / current_price * 100, 2
            )
...
def estimate_win_rate_from_signals(
    ...
    # Fundamental quality
    avg_fund = (fund_signal.value_score + fund_signal.quality_score) / 2
    ...
    vol = risk_metrics.get("volatility_annual", 0.30)
...
def compute_composite_score(
    ...
    # Fundamental component: average of value, quality, safety
    fund_score = (fund_signal.value_score + fund_signal.quality_score + fund_signal.safety_score) / 3
    ...
    # Growth component
    growth_score = fund_signal.growth_score
...
def compute_risk_assessment(
    ...
    if atr > 0:
        sig.stop_loss_price = round(current_price - stop_loss_atr_mult * atr, 3)
    else:
        sig.stop_loss_price = round(current_price * 0.95, 3)  # 5% default

    # ── Position risk
    position_value = portfolio_value * proposed_position_pct
    loss_per_share = current_price - sig.stop_loss_price
    if current_price > 0:
        sig.position_risk_pct = round(loss_per_share / current_price * proposed_position_pct * 100, 2)
    sig.max_loss_amount = round(position_value * (loss_per_share / (current_price + 1e-10)), 2)

    # ── Concentration check (Markowitz)
    max_single = risk_limits.get("max_single_position_pct", 0.10)
    if proposed_position_pct > max_single:
        sig.concentration_ok = False
        sig.veto_reasons.append(
            f"持仓集中度超限: {proposed_position_pct:.1%} > {max_single:.1%}"
        )
    ...
    if vol > 0.60:  # >60% annualized vol = extreme
        sig.volatility_ok = False
        sig.veto_reasons.append(
            f"波动率极端: 年化波动 {vol:.1%}，VaR95 = {var_95:.2%}/day"
        )

    # ── Taleb fat-tail adjustment: true risk ≈ 2× VaR
    if var_95 < 0:
        sig.portfolio_var_impact = round(abs(var_95) * 2 * proposed_position_pct * 100, 2)
